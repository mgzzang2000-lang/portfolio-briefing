...
EARLY_WINDOW_END_HHMM = (9, 27)  # 스토캐스틱RSI 계산 가능 시점(27분, ≈09:27) —
# 그 이후는 check_1min_timing(분봉 기반)이 이어받으므로 섀도우D는 그 전까지만 동작
...
# ── 섀도우 D: 오프닝 레인지 브레이크아웃(ORB, 틱 기반) — 장 극초반(09:00~09:27) 전용 ──
...
def scan_shadow_d(token, stocks, kospi_set):
    now = datetime.now(KST)
    if (now.hour, now.minute) >= EARLY_WINDOW_END_HHMM or (now.hour, now.minute) < (9, OPENING_RANGE_MINUTES):
        return []
